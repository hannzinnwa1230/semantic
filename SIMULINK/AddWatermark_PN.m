function [img_watermark,realwatermark] = AddWatermark_PN(originalimage,originalwatermark,alpha)
%% 
% 功能：对图像进行水印添加
% input：
% originalimage：原始图像
...
for i = 1:size_wm(1)
    for j = 1:size_wm(2)
        ...
        [U,S,V] = svd(A);
        if(watermark(i,j) == 0)
            S(3,3) = (1-alpha)*S(3,3)+alpha*S(2,2);
        elseif(watermark(i,j) == 1)
            S(3,3) = (1-alpha)*S(3,3)+alpha*S(4,4);
        end        
        A_w = U*S*V';
        A_w = mat2cell(A_w,4,4);
        B_w(i,j) = A_w;
