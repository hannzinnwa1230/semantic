...
while (searchOffset + minPktLen) <= rxWaveformLen    
    ...
    [rxPSDU,eqSym] = wlanNonHTDataRecover(...
     ...
     if(isequal(size(eqSym),[48,98]))
        eqsym1(:,:,eqsymnum1) = eqSym;  
        eqsymnum1 = eqsymnum1+1;
     end
     if(isequal(size(eqSym),[48,195]))
        eqsym2(:,:,eqsymnum2) = eqSym;  
        eqsymnum2 = eqsymnum2+1;
     end
     if(isequal(size(eqSym),[48,130]))
        eqsym3(:,:,eqsymnum3) = eqSym;  
        eqsymnum3 = eqsymnum3+1;
     end
